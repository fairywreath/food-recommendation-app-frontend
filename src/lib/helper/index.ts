...
export const generateSrcset = (image: object[]) =>
  image &&
  image
    .map((img: any) => (img?.width ? `${img.url} ${img.width}w` : `${img.url}`))
    .toString();
...
export const errorHandler = (form: any, key: string) =>
  form?.error && form?.errors[key];

export const removeEmptyElements = (items: object[]) =>
  items.filter((n: any) => n);
